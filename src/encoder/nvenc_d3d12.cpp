...
NvencD3D12::~NvencD3D12() { Shutdown(); }

bool NvencD3D12::Initialize(NvencSession* session, uint32_t buffer_count) {
    if (!session || !session->IsInitialized()) return false;
    session = session;
...
bool NvencD3D12::RegisterTexture(ID3D12Resource* texture, uint32_t width, uint32_t height,
    ...
    if (!session || !texture) return false;
...
bool NvencD3D12::MapInputTexture(uint32_t index) {
    ...
    auto& texture = textures[index];
    if (texture.is_mapped) return true;
    if (!texture.registered_ptr) return false;
...
BitstreamBuffer* NvencD3D12::GetBitstreamBuffer(uint32_t index) {
    if (index >= bitstream_buffers.size()) return nullptr;
