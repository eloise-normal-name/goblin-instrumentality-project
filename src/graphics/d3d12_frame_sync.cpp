...
void D3D12FrameSync::WaitForGpu() {
...
void D3D12FrameSync::MoveToNextFrame(uint32_t previous_frame_index, uint32_t next_frame_index) {
...
uint64_t D3D12FrameSync::SignalFenceForFrame(uint32_t frame_index) {
...
void D3D12FrameSync::SetFenceEvent(uint64_t value, HANDLE event) {
	if (!fence || !event)
		return;
