...
class SharedTexture {
   public:
    ...
    HANDLE GetSharedHandle() const { return shared_handle; }
    uint32_t GetWidth() const { return width; }
    uint32_t GetHeight() const { return height; }
    DXGI_FORMAT GetFormat() const { return format; }
    ResourceState GetCurrentState() const { return current_state; }
    void SetCurrentState(ResourceState state) { current_state = state; }
...
D3D12_RESOURCE_BARRIER CreateTransitionBarrier(ID3D12Resource* resource, D3D12_RESOURCE_STATES before,
