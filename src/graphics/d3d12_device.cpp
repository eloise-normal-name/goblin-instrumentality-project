...
bool D3D12Device::Initialize(const DeviceConfig& cfg) {
    ...
    if (!CreateSwapChain(cfg.window_handle, cfg.frame_width, cfg.frame_height)) return false;
...
bool D3D12Device::CreateDevice() {
    ...
    for (UINT i = 0; factory->EnumAdapterByGpuPreference(i, DXGI_GPU_PREFERENCE_HIGH_PERFORMANCE,
        ...
        if (SUCCEEDED(D3D12CreateDevice(adapter1.Get(), D3D_FEATURE_LEVEL_12_0, IID_PPV_ARGS(&device)))) {
            adapter1.As(&adapter);
            break;
        }
    }
