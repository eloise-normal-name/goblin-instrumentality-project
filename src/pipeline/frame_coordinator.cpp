...
FrameCoordinator::~FrameCoordinator() { Shutdown(); }
...
bool FrameCoordinator::Initialize(D3D12Device* dev, const PipelineConfig& cfg) {
    ...
    if (!commands.Initialize(device->GetDevice())) return false;
    if (!CreateEncoderTexture()) return false;
...
bool FrameCoordinator::EndFrame() {
    ...
    commands.TransitionResource(device->GetCurrentRenderTarget(), D3D12_RESOURCE_STATE_RENDER_TARGET,
...
bool FrameCoordinator::EncodeFrame(FrameData& output) {
    ...
    if (!SubmitFrameToEncoder()) return false;
    if (!RetrieveEncodedFrame(output)) return false;
...
bool FrameCoordinator::InitializeEncoder() {
    if (!nvenc_session.LoadLibrary()) return false;
    if (!nvenc_session.OpenSession(device->GetDevice())) return false;

    if (!nvenc_d3d12.Initialize(&nvenc_session, device->GetBufferCount())) return false;
    ...
    if (!nvenc_config.Initialize(&nvenc_session, enc_config)) return false;
    if (!nvenc_config.InitializeEncoder()) return false;
