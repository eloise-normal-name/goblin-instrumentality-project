...
When an ExecPlan instructs work in a specific git worktree, that worktree path is mandatory session scope. Do not run edits, builds, tests, or cleanup commands in another worktree or the main checkout unless the user explicitly approves the switch. If a tool context reset points to a different directory, stop and reconfirm before continuing.
