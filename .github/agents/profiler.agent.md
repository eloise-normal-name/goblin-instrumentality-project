...
1. **Measure** — run VSDiagnostics.exe CLI sessions for CPU usage, memory allocation, and file I/O against the deterministic headless workload (30 frames, RelWithDebInfo build).
...
- **Workload**: `bin\RelWithDebInfo\goblin-stream.exe --headless` (exits after 30 frames — deterministic, steady-state encode loop)
...
# Memory
& $vsdiag start 1 /launch:"bin\RelWithDebInfo\goblin-stream.exe" /launchArgs:"--headless" /loadConfig:"$configs\MemoryUsage.json"
& $vsdiag stop  1 /output:"$out\memory_$stamp.diagsession"

# File I/O
& $vsdiag start 1 /launch:"bin\RelWithDebInfo\goblin-stream.exe" /launchArgs:"--headless" /loadConfig:"$configs\FileIO.json"
...
- Do not run Debug builds for baseline profiling — use RelWithDebInfo for optimized measurements with symbols.
