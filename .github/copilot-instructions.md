...
If there is ever a conflict between this file and `QUICK_REFERENCE.md`, the Quick Reference is the source of truth and takes precedence.
...
- **ComPtr Usage** (**❌ NOT CI-enforced**):
  ...
  - WRL address-of nuance: The pattern `*&com_ptr` used in this repo is intentional because WRL `ComPtr` overloads `operator&`. Do not simplify `*&device.device` to `&device.device`.
...
### Formatting from CLI (PowerShell)

- Format all C/C++ sources in-place under `src` and `include`:

```powershell
"C:\Program Files\Microsoft Visual Studio\18\Community\VC\Tools\Llvm\x64\bin\clang-format.exe" -i (Get-ChildItem -Recurse src,include -Include *.cpp,*.h,*.hpp,*.ixx | ForEach-Object { $_.FullName })
```

- Format only staged files:

```powershell
git diff --name-only --cached | Where-Object { $_ -match '\\.(cpp|h|hpp|ixx)$' } | ForEach-Object { & "C:\Program Files\Microsoft Visual Studio\18\Community\VC\Tools\Llvm\x64\bin\clang-format.exe" -i $_ }
```
...
Additional quick sanity runs (headless):

```powershell
powershell -ExecutionPolicy Bypass -File scripts/agent-wrap.ps1 -Command ".\bin\Debug\goblin-stream.exe --headless" -TimeoutSec 60 -PrintOutput
powershell -ExecutionPolicy Bypass -File scripts/agent-wrap.ps1 -Command ".\bin\Release\goblin-stream.exe --headless" -TimeoutSec 60 -PrintOutput
```
